...
import pickle
pca = pickle.load(open('Sipakmed_cluster_cnn3_pcaTransform','rb'))
clf = pickle.load(open('Sipakmed_cluster_cnn3_pca','rb'))
...
if uploaded_file is not None:
    ...
    for img in imgs:
        ...
        # get features
        features = model(img_as_tensor).squeeze(dim=0).sum(1).sum(1).detach().numpy() #sum pooling, paper says good extractor
